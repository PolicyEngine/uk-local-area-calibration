...
# Read the files
df_age = pd.read_csv('raw_age.csv', 
                skiprows=6,
                thousands=',')
...
if 'total_income_count' in df_income.columns:
    df_income['total_income_count'] = pd.to_numeric(df_income['total_income_count'], errors='coerce')

if 'total_income_amount' in df_income.columns:
    df_income['total_income_amount'] = pd.to_numeric(df_income['total_income_amount'], errors='coerce')
