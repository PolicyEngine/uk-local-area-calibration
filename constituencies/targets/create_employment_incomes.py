...
def find_and_replace_zero_populations(result_df_copy, total_income) -> pd.DataFrame:
    ...
    for zero_constituency in constituencies_with_zero_population:
        try:
            ...
            current_constituency_data = total_income[total_income['code'] == zero_constituency]
            
            if current_constituency_data.empty:
                print(f"Warning: No data found for constituency {zero_constituency} in total_income")
                continue
                
            current_total_income = current_constituency_data['total_income_count'].values[0]
            ...
            differences = np.abs(other_constituencies['total_income_count'] - current_total_income)
            ...
            zero_const_rows = result_df[result_df['constituency_code'] == zero_constituency]
            
            if zero_const_rows.empty:
                print(f"Warning: No rows found for constituency {zero_constituency} in result_df")
                continue
            ...
            for _, zero_row in zero_const_rows.iterrows():
                ...
                matching_row = result_df[
                    ...
                    (result_df['income_lower_bound'] == zero_row['income_lower_bound']) &
                    (result_df['income_upper_bound'] == zero_row['income_upper_bound'])
                ]
                
                if matching_row.empty:
                    print(f"Warning: No matching income band found for constituency {nearest_constituency}")
                    continue
                ...
                result_df.loc[mask, 'population_count'] = matching_row['population_count'].values[0]
                result_df.loc[mask, 'total_earning'] = matching_row['total_earning'].values[0]
